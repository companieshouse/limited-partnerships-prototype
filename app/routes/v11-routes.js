...
// GOV One Login Sign in
  ...
  router.post('/gov-onelogin-email', function(request, response) {
    ...
    if (govOneLoginEmail == "someone@test.com"){
        response.redirect("/v11/not-eligible")
    } 
    else {
        response.redirect("/v11/gov-onelogin-password")
    }
  })
...
router.post('/gov-onelogin-enter-code', function(request, response) {
    ...
    if (registrationOrTransition == "post"){
        response.redirect("/v11/chs/company-info?authorised")
    }     
...
router.post('/secure-register-filter', function(request, response) {
    ...
    if (secureRegister == "yes"){
        response.redirect("/v11/use-paper")
    }
    else {
        response.redirect("/v11/registration/which-type")
    }
  })
  ...
  router.post('/which-type', function(request, response) {
    ...
    if (registerType == "RegisterPflp"){
        response.redirect("/v11/registration/pflp-name")
    }  
    else if (registerType == "RegisterPflpSco"){
        response.redirect("/v11/registration/scottish-pflp-name")
     }    
    else if (registerType == "registerSlp"){
        response.redirect("/v11/registration/slp-name")
     } 
     else if (registerType == "RegisterSqp"){
        response.redirect("/v11/registration/sqp-name")
     } 
     else {
        response.redirect("/v11/registration/lp-name")
    }
})
...
router.post(['/lp-name', '/pflp-name', '/slp-name', '/sqp-name'], (request, response) => {
    ...
    if (lpChooseName && lpChooseName.toLowerCase().includes("harrods")) {
        response.redirect("/v11/registration/same-as-name");
    } else if (lpChooseName && lpChooseName.toLowerCase().includes("bank")) {
        response.redirect("/v11/registration/sensitive-name");
    } else if (request.path === '/lp-name' && registrationOrTransition === "post") {
        response.redirect("/v11/manage/date-of-change-lpname-update");
    } else {
...
// LP number confirm - transition or post-transition
  ...
  router.post('/company-number', function(request, response) {
    ...
    if (lpNumber == "LP999999"){
        response.redirect("/v11/transition-already-filed")
    } 
    else {
        response.redirect("/v11/correct-company")
    }
  })
...
router.post('/correct-company', function(request, response) {
  ...
  router.post('/limited-partnership-rea', function(request, response) {
    ...
    if (registrationOrTransition == "registration"){
        response.redirect("/v11/limited-partnership-jurisdiction")
    } 
    else {
        response.redirect("/v11/limited-partnership-roa")
    }
  })

  router.post('/company-number', function (req, res) {
    res.redirect('/v11/correct-company')
  })
...
router.post('/general-partner-choice', function(request, response) {
    ...
    if (generalPartner == "person"){
        response.redirect("/v11/gp-person")
    } else {
        response.redirect("/v11/gp-legal-entity")
    }
})
...
router.post('/gp-add-another', function(request, response) {
    ...
    if (registrationOrTransition == "post") {
        response.redirect("/v11/manage/confirmation-additional-gp")
    } else if (addAnotherGP == "addPersonGP") {
        response.redirect("/v11/gp-person")
    } else if (addAnotherGP == "addEntityGP") {
        response.redirect("/v11/gp-legal-entity")
    } else {
        response.redirect("/v11/limited-partner-section")
    }
...
router.post('/limited-partner-choice', function(request, response) {
    ...
    if (limitedPartner == "person"){
        response.redirect("/v11/lp-person")
    } else {
        response.redirect("/v11/lp-legal-entity")
    }
})
...
router.post('/lp-add-another', function(request, response) {
    ...
    if (addAnotherLP === "addPersonLP") {
     ...
     {
        response.redirect("/v11/manage/confirmation-add-lp");
    }
    else {
        response.redirect("/v11/check-your-answers");
    }
});
...
router.post('/registration-or-transition', function(request, response) {
    var registrationOrTransition = request.session.data['registrationOrTransition']
    if (registrationOrTransition == "registration"){
        response.redirect("/v11/registration/registration-start-page")
    }
    else if (registrationOrTransition == "transition"){
...
router.post('/limited-partnership-roa', function(request, response) {
    response.redirect('/v11/limited-partnership-roa-choose-address')
});

//Registered office address (choose address)
router.post('/limited-partnership-roa-choose-address', function(request, response) {
    response.redirect('/v11/limited-partnership-roa-confirm-address')
})
...
router.post('/limited-partnership-roa-confirm-address', function(request, response) {
   ...
   var registrationOrTransition = request.session.data['registrationOrTransition']
   if (registrationOrTransition == "post"){
      response.redirect("/v11/filing-confirmation")
   }
      else if (registrationOrTransition == "transition"){
        response.redirect("/v11/general-partner-section")
   } 
   else {
      response.redirect("/v11/limited-partnership-ppob")
  }
})
...
router.post('/limited-partnership-roa-manual', function(request, response) {
  ...
  if (from === 'roa-update') {
    response.redirect('/v11/manage/date-of-change-roa-update'); // replace with your actual route
  } else {
    response.redirect('/v11/limited-partnership-roa-confirm-address');
  }
});
...
router.post('/limited-partnership-terms', function(request, response) {
    ...
    var registrationOrTransition = request.session.data['registrationOrTransition']
    if (registrationOrTransition == "post"){
        response.redirect("/v11/manage/date-of-change-term-update")
    }
    else if (registrationOrTransition == "transition"){
        response.redirect("/v11/general-partner-section")
    }
...
router.post('/limited-partnership-sic', function(request, response) {
    ...
    if (checkSIC === "no") {
        response.redirect('/v11/confirmation-statement/sic-check')
    } else {
        response.redirect('/v11/general-partner-section')
    }
})
...
router.post('/confirmation-statement-date', function(request, response) {
    ...
    if (changeDateOfCS === "yes") {
        response.redirect('/v11/confirmation-statement/check-your-answers-change-cs-date')
    } else {
        response.redirect('/v11/confirmation-statement/sic-check')
    }
})
...
if (pscStatement === "No, there is no person identified as a PSC in relation to the limited partnership") {
  response.redirect('/v11/check-your-answers')
        ...
        response.redirect('/v11/pscs/psc-choice')
    }
})
...
router.post('/psc-choice', function(request, response) {
    ...
    if (pscType == "entity"){
        response.redirect("/v11/pscs/psc-legal-entity")
    }
    else if (pscType == "orp"){
...
router.get('/v11/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-manual', function (req, res) {
    res.render('v11/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-manual', {
...
router.post('/gp-legal-entity-poa-where', function(request, response) {
    ...
    if (gpLegalEntityPOAWhere == "gpLegalEntityPOAUK"){
        response.redirect("/v11/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-postcode-look-up")
    }
    else {
        response.redirect("/v11/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-manual")
    }
  })
...
router.post('/gp-legal-entity-poa-postcode-look-up', function(request, response) {
    response.redirect('/v11/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-choose-address')
})


  router.post('/gp-legal-entity-poa-choose-address', function(request, response) {
    response.redirect('/v11/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-confirm-address')
})
...
router.get('/v11/address-pages/gp-person-ura/gp-person-ura-manual', function (req, res) {
    res.render('v11/address-pages/gp-person-ura/gp-person-ura-manual', {
      from: req.query.from // This makes `from` available in Nunjucks
    });
  });
...
router.post('/gp-person-ura-where', function(request, response) {
    ...
    if (gpPersonURAWhere == "gpPersonURAUK"){
        response.redirect("/v11/address-pages/gp-person-ura/gp-person-ura-postcode-look-up")
    }
    else {
        response.redirect("/v11/address-pages/gp-person-ura/gp-person-ura-manual")
    }
  })
...
router.post('/gp-person-ura-postcode-look-up', function(request, response) {
    response.redirect('/v11/address-pages/gp-person-ura/gp-person-ura-choose-address')
})


  router.post('/gp-person-ura-choose-address', function(request, response) {
    response.redirect('/v11/address-pages/gp-person-ura/gp-person-ura-confirm-address')
})
...
//This GET code is used to preppoluate the values when the user goes from Confirm the address to Manuallu entering it
router.get('/v11/address-pages/gp-person-ca/gp-person-ca-manual', function (req, res) {
    res.render('v11/address-pages/gp-person-ca/gp-person-ca-manual', {
      from: req.query.from // This makes `from` available in Nunjucks
    });
  });
...
router.post('/gp-person-ca-where', function(request, response) {
    ...
    if (gpPersonCAWhere == "gpPersonCAUK"){
        response.redirect("/v11/address-pages/gp-person-ca/gp-person-ca-postcode-look-up")
    }
    else {
        response.redirect("/v11/address-pages/gp-person-ca/gp-person-ca-manual")
    }
  })
...
router.post('/gp-person-ca-postcode-look-up', function(request, response) {
    response.redirect('/v11/address-pages/gp-person-ca/gp-person-ca-choose-address')
})


  router.post('/gp-person-ca-choose-address', function(request, response) {
    response.redirect('/v11/address-pages/gp-person-ca/gp-person-ca-confirm-address')
})
...
//This GET code is used to preppoluate the values when the user goes from Confirm the address to Manuallu entering it
router.get('/v11/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-manual', function (req, res) {
    res.render('v11/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-manual', {
      from: req.query.from // This makes `from` available in Nunjucks
    });
  });
...
router.post('/lp-legal-entity-poa-where', function(request, response) {
    ...
    if (lpLegalEntityPOAWhere == "lpLegalEntityPOAUK"){
        response.redirect("/v11/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-postcode-look-up")
    }
    else {
        response.redirect("/v11/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-manual")
    }
  })
...
router.post('/lp-legal-entity-poa-postcode-look-up', function(request, response) {
    response.redirect('/v11/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-choose-address')
})


  router.post('/lp-legal-entity-poa-choose-address', function(request, response) {
    response.redirect('/v11/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-confirm-address')
})
...
//This GET code is used to preppoluate the values when the user goes from Confirm the address to Manuallu entering it
router.get('/v11/address-pages/lp-person-ura/lp-person-ura-manual', function (req, res) {
    res.render('v11/address-pages/lp-person-ura/lp-person-ura-manual', {
      from: req.query.from // This makes `from` available in Nunjucks
    });
  });
...
router.post('/lp-person-ura-where', function(request, response) {
    ...
    if (lpPersonURAWhere == "lpPersonURAUK"){
        response.redirect("/v11/address-pages/lp-person-ura/lp-person-ura-postcode-look-up")
    }
    else {
        response.redirect("/v11/address-pages/lp-person-ura/lp-person-ura-manual")
    }
  })
...
router.post('/lp-person-ura-postcode-look-up', function(request, response) {
    response.redirect('/v11/address-pages/lp-person-ura/lp-person-ura-choose-address')
})


  router.post('/lp-person-ura-choose-address', function(request, response) {
    response.redirect('/v11/address-pages/lp-person-ura/lp-person-ura-confirm-address')
})
...
router.post('/lp-person-ura-confirm-address', function(request, response) {
    ...
    var registrationOrTransition = request.session.data['registrationOrTransition']
    if (registrationOrTransition == "post"){
        response.redirect("/v11/manage/check-your-answers-add-lp")
    }
    else {
        response.redirect("/v11/lp-add-another")
    }
  })
...
router.post('/check-sic', function(request, response) {
  ...
  //Update a partner
    ...
    if (changeURA == "yes"){
        response.redirect("/v11/address-pages/gp-person-ura/gp-person-ura-where")
    }
    else {
        response.redirect("/v11/address-pages/gp-person-ca/gp-person-ca-manual")
    }
