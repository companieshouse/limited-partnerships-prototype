...
router.use('/', (req, res, next) => {
  ...
  //CHS sign in 
    ...
    router.get('/chs-sign-in-fileForThisCompany', function(request, response) {
        ...
        response.redirect("v15/chs/chs-sign-in")
...
// GOV One Login Sign in
  ...
  router.post('/gov-onelogin-email', function(request, response) {
        ...
        response.redirect("v15/gov-onelogin-password")
    ...
    if (registrationOrTransition == "post"){
        ...
        // email address check for agent 
            ...
            if (govOneLoginEmail == "someone@email.com"){
                response.redirect("v15/not-eligible")
            } 
                ...
                if (fileForThisCompany = true){
                    ...
                    response.redirect("v15/chs/company-info?authorised")
                } 
                ...
                //User has selected sign in, but not selected File for a company button
                    ...
                    response.redirect("v15/chs/company-info?signedIn")
         ...
         response.redirect("v15/starting-new")
     } 
...
router.post('/sign-out', function(request, response) {
    ...
    if (signingOut == "yes"){
        ...
        response.redirect("v15/signed-out")
    }     
...
router.post('/secure-register-filter', function(request, response) {
    ...
    if (secureRegister == "yes"){
        response.redirect("v15/use-paper")
    }
    else {
        response.redirect("v15/registration/which-type")
    }
  })
  ...
  router.post('/which-type', function(request, response) {
    ...
    if (registerType == "RegisterPflp"){
        response.redirect("v15/registration/pflp-name")
    }  
    else if (registerType == "RegisterPflpSco"){
        response.redirect("v15/registration/scottish-pflp-name")
     }    
    else if (registerType == "registerSlp"){
        response.redirect("v15/registration/slp-name")
     } 
     else if (registerType == "RegisterSqp"){
        response.redirect("v15/registration/sqp-name")
     } 
     else {
        response.redirect("v15/registration/lp-name")
    }
})
...
router.post('/correct-company', function(request, response) {
    ...
    if (registrationOrTransition == "post") {
        response.redirect("v15/manage/limited-partnership-overview-tabs-wf")
    } 
    else if (registrationOrTransition == "cs") {
        response.redirect("v15/manage/file-confirmation-statement")
    }
    else {
        response.redirect("v15/limited-partnership-rea")
    }
  ...
  router.post('/limited-partnership-rea', function(request, response) {
    ...
    if (registrationOrTransition == "registration"){
        response.redirect("v15/limited-partnership-jurisdiction")
    } 
    else {
        response.redirect("v15/limited-partnership-roa")
    }
  })

  router.post('/company-number', function (req, res) {
    res.redirect('v15/correct-company')
  })
...
router.post('/general-partner-choice', function(request, response) {
    ...
    if (generalPartner == "person"){
        response.redirect("v15/gp-person")
    } else {
...
router.post('/limited-partner-choice', function(request, response) {
    ...
    if (limitedPartner == "person"){
        ...
        response.redirect("v15/lp-legal-entity")
    }
})
...
router.post('/lp-add-another', function(request, response) {
    ...
    if (addAnotherLP === "addPersonLP") {
     ...
     {
        response.redirect("v15/manage/confirmation-add-lp");
    }
    else {
...
router.post('/registration-or-transition', function(request, response) {
    var registrationOrTransition = request.session.data['registrationOrTransition']
    if (registrationOrTransition == "registration"){
        response.redirect("v15/registration/registration-start-page")
    }
    else if (registrationOrTransition == "transition"){
        response.redirect("v15/transition-start-page")
    }
       else if (registrationOrTransition == "cs"){
        response.redirect("v15/confirmation-statement/start")
    }
    else {
        response.redirect("v15/post-transition-guidance")
    }
  })
...
router.post('/start', function(request, response) {
    ...
    if (registrationOrTransition == "registration"){
        ...
        response.redirect("v15/post-transition-start-page")
    }
  })
...
router.post('/limited-partnership-jurisdiction', function(request, response) {
  var userHasSignedOut = request.session.data['userHasSignedOut']
    ...
    response.redirect('v15/limited-partnership-roa-confirm-address')
...
router.post('/limited-partnership-roa', function(request, response) {
    response.redirect('v15/limited-partnership-roa-choose-address')
});
...
router.post('/limited-partnership-roa-confirm-address', function(request, response) {
   ...
   var registrationOrTransition = request.session.data['registrationOrTransition']
   if (registrationOrTransition == "post"){
      response.redirect("v15/filing-confirmation")
   }
    else if (registrationOrTransition == "transition"){
    ...
    if (userHasSignedOut == true) {
            ...
            response.redirect("v15/limited-partnership-ppob-confirm-address")

        }
        else{

            response.redirect("v15/limited-partnership-ppob")
...
router.post('/limited-partnership-roa-manual', function(request, response) {
  ...
  if (from === 'roa-update') {
    response.redirect('v15/manage/date-of-change-roa-update'); // replace with your actual route
  } else {
    response.redirect('v15/limited-partnership-roa-confirm-address');
  }
});
...
//response.redirect("v15/general-partner-section");
      ...
      //  response.redirect("v15/general-partner-section");
              response.redirect("v15/limited-partnership-ppob-choose-address");
...
// Starting new - v15 added Filing history pages into flow
...
router.post('/limited-partnership-terms', function(request, response) {
    ...
    var registrationOrTransition = request.session.data['registrationOrTransition']
    if (registrationOrTransition == "post"){
        response.redirect("v15/manage/date-of-change-term-update")
    }
    else if (registrationOrTransition == "transition"){
        response.redirect("v15/general-partner-section")
    }
    else {
        response.redirect("v15/limited-partnership-sic-2")
    }
})
...
router.post('/confirmation-statement-date', function(request, response) {
    ...
    if (changeDateOfCS === "yes") {
        response.redirect('v15/confirmation-statement/check-your-answers-change-cs-date')
    } else {
        response.redirect('v15/confirmation-statement/sic-check')
    }
})
...
router.post('/psc-choice', function(request, response) {
    ...
    if (pscType == "entity"){
        response.redirect("v15/pscs/psc-legal-entity")
    }
    else if (pscType == "orp"){
        response.redirect("v15/pscs/psc-orp")
    }
    else {
        response.redirect("v15/pscs/psc-protected-details")
    }
  })
  ...
  router.post('/psc-protected-details', function(request, response) {
    ...
    if (pscProtection == "yes"){
        ...
        response.redirect('v15/pscs/psc-protected-details-confirm')

    }
    else if (pscProtection == "no"){
        response.redirect('v15/pscs/psc-person-alt-1')
    }
...
/* Nature of controls - *** Indivual *** control page */
  ...
  router.post('/psc-person-alt-nature', function(request, response) {
    ...
    if (typeIndividualPSCNOC.includes("nocIndividual")){
        response.redirect('v15/pscs/psc-person-alt-nature-individual')
    } 
    else if (typeIndividualPSCNOC.includes("nocFirm")){
        response.redirect('v15/pscs/psc-person-alt-nature-firm')
    } 
    else if (typeIndividualPSCNOC.includes("nocTrust")) {
        response.redirect('v15/pscs/psc-person-alt-nature-trust')
    }
    else {
        response.redirect('v15/pscs/pscs-addresses/psc-person-ura/psc-person-ura-where')
    }
...
/* RLE Nature of controls */
  ...
  router.post('/psc-legal-entity-nature', function(request, response) {
    ...
    if (typeRLENOC.includes("nocRLERLE")){
        response.redirect('v15/pscs/psc-legal-entity-nature-rle')
    } 
    else if (typeRLENOC.includes("nocRLEFirm")){
        response.redirect('v15/pscs/psc-legal-entity-nature-firm')
    } 
    else if (typeRLENOC.includes("nocRLETrust")) {
        response.redirect('v15/pscs/psc-legal-entity-nature-trust')
    }
    else {
        response.redirect('v15/pscs/pscs-addresses/psc-legal-entity-poa/psc-legal-entity-poa-where')
    }
...
/* ORP Nature of controls */
  ...
  router.post('/psc-orp-nature', function(request, response) {
    ...
    if (typeORPNOC.includes("nocORP")){
        response.redirect('v15/pscs/psc-orp-nature-orp')

    }
    else if (typeORPNOC.includes("nocORPFirm")){
        response.redirect('v15/pscs/psc-orp-nature-firm')
    } 
    else if (typeORPNOC.includes("nocORPTrust")) {
        response.redirect('v15/pscs/psc-orp-nature-trust')
    }
    else {
         response.redirect('v15/pscs/pscs-addresses/psc-orp-poa/psc-orp-poa-where')
...
router.post('/payment-placeholder', function(request, response) {
  ...
  if (referrer.includes('?change-type')) {
    response.redirect('v15/manage/confirmation-change-type')
  } 
  else if (referrer.includes('?change-name')) {
    response.redirect('v15/manage/confirmation-change-name')
  }
  else {
    response.redirect("v15/confirmation-page")
  }
})
...
router.get('/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-manual', function (req, res) {
    res.render('v15/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-manual', {
...
router.post('/gp-legal-entity-poa-where', function(request, response) {
    ...
    if (gpLegalEntityPOAWhere == "gpLegalEntityPOAUK"){
        response.redirect("v15/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-postcode-look-up")
    }
    else {
        response.redirect("v15/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-manual")
    }
  })
...
router.post('/gp-legal-entity-poa-postcode-look-up', function(request, response) {
    response.redirect('v15/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-choose-address')
})


  router.post('/gp-legal-entity-poa-choose-address', function(request, response) {
    response.redirect('v15/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-confirm-address')
})
...
router.get('/address-pages/gp-person-ura/gp-person-ura-manual', function (req, res) {
    res.render('v15/address-pages/gp-person-ura/gp-person-ura-manual', {
      from: req.query.from // This makes `from` available in Nunjucks
    });
  });
...
router.post('/gp-person-ura-where', function(request, response) {
    ...
    if (gpPersonURAWhere == "gpPersonURAUK"){
        response.redirect("v15/address-pages/gp-person-ura/gp-person-ura-postcode-look-up")
    }
    else {
        response.redirect("v15/address-pages/gp-person-ura/gp-person-ura-manual")
    }
  })
...
router.post('/gp-person-ura-postcode-look-up', function(request, response) {
    response.redirect('v15/address-pages/gp-person-ura/gp-person-ura-choose-address')
})


  router.post('/gp-person-ura-choose-address', function(request, response) {
    response.redirect('v15/address-pages/gp-person-ura/gp-person-ura-confirm-address')
})
...
router.get('/address-pages/gp-person-ca/gp-person-ca-manual', function (req, res) {
    res.render('v15/address-pages/gp-person-ca/gp-person-ca-manual', {
      from: req.query.from // This makes `from` available in Nunjucks
    });
  });
...
router.post('/gp-person-ca-where', function(request, response) {
    ...
    if (gpPersonCAWhere == "gpPersonCAUK"){
        response.redirect("v15/address-pages/gp-person-ca/gp-person-ca-postcode-look-up")
    }
    else {
        response.redirect("v15/address-pages/gp-person-ca/gp-person-ca-manual")
    }
  })
...
router.post('/gp-person-ca-postcode-look-up', function(request, response) {
    response.redirect('v15/address-pages/gp-person-ca/gp-person-ca-choose-address')
})


  router.post('/gp-person-ca-choose-address', function(request, response) {
    response.redirect('v15/address-pages/gp-person-ca/gp-person-ca-confirm-address')
})
...
router.get('/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-manual', function (req, res) {
    res.render('v15/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-manual', {
      from: req.query.from // This makes `from` available in Nunjucks
    });
  });
...
router.post('/lp-legal-entity-poa-where', function(request, response) {
    ...
    if (lpLegalEntityPOAWhere == "lpLegalEntityPOAUK"){
        response.redirect("v15/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-postcode-look-up")
    }
    else {
        response.redirect("v15/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-manual")
    }
  })
...
router.post('/lp-legal-entity-poa-postcode-look-up', function(request, response) {
    response.redirect('v15/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-choose-address')
})


  router.post('/lp-legal-entity-poa-choose-address', function(request, response) {
    response.redirect('v15/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-confirm-address')
})
...
router.get('v15/lp-person', function (req, res) {
    res.render('v15/lp-person', {
      from: req.query.from // This makes `from` available in Nunjucks
    });
  });
  ...
  router.post('/lp-person', function(request, response) {
    ...
    var registrationOrTransition = request.session.data['registrationOrTransition']
       ...
       if (registrationOrTransition == "post"){
        ...
        response.redirect("v15/manage/change-lp-ura")
        
       }
       else {

        response.redirect("v15/address-pages/lp-person-ura/lp-person-ura-where")
       }
...
//This GET code is used to preppoluate the values when the user goes from Confirm the address to Manuallu entering it
router.get('v15/address-pages/lp-person-ura/lp-person-ura-manual', function (req, res) {
    res.render('v15/address-pages/lp-person-ura/lp-person-ura-manual', {
      from: req.query.from // This makes `from` available in Nunjucks
    });
  });
...
router.post('/lp-person-ura-where', function(request, response) {
    ...
    if (lpPersonURAWhere == "lpPersonURAUK"){
        response.redirect("v15/address-pages/lp-person-ura/lp-person-ura-postcode-look-up")
    }
    else {
        response.redirect("v15/address-pages/lp-person-ura/lp-person-ura-manual")
    }
  })
...
router.post('/lp-person-ura-postcode-look-up', function(request, response) {
    response.redirect('v15/address-pages/lp-person-ura/lp-person-ura-choose-address')
})


  router.post('/lp-person-ura-choose-address', function(request, response) {
    response.redirect('v15/address-pages/lp-person-ura/lp-person-ura-confirm-address')
})
...
router.post('/lp-person-ura-confirm-address', function(request, response) {
    ...
    var registrationOrTransition = request.session.data['registrationOrTransition']
    if (registrationOrTransition == "post"){
        response.redirect("v15/manage/date-of-change-partner-lp-person")
    }
    else {
        response.redirect("v15/lp-add-another")
    }
...
router.post('/check-sic', function(request, response) {
    ...
    var checkSIC = request.session.data['checkSIC']
    if (checkSIC == "yes"){
        response.redirect("v15/confirmation-statement/submit-confirmation-statement")
    }
    else {
        response.redirect("v15/limited-partnership-sic-2")
    }
  ...
  //Update a partner address ura
    ...
    if (changeURA == "yes"){
        response.redirect("v15/address-pages/gp-person-ura/gp-person-ura-where")
    }
    else {

        response.redirect("v15/manage/change-uca")
    ...
    if (changeUCA == "yes"){
        ...
        response.redirect("v15/manage/date-of-change-partner-update")
    ...
    if (changePPB == "yes"){
        response.redirect("v15/address-pages/gp-legal-entity-poa/gp-legal-entity-poa-where")
    }
    else {

        response.redirect("v15/manage/date-of-change-partner-le-update")
        // nneds to be the radios page response.redirect("/v12/address-pages/gp-person-ca/gp-person-ca-manual")
    }
  })
    ...
    if (changePOA == "yes"){
        response.redirect("v15/address-pages/lp-legal-entity-poa/lp-legal-entity-poa-where")
    }
    else {

        response.redirect("v15/manage/date-of-change-partner-lp-update")
        // nneds to be the radios page response.redirect("/v12/address-pages/gp-person-ca/gp-person-ca-manual")
    }
  })
